...
void PathConstraint::setPath(PathConstraintPathBase *path)
{
    ...
    if (m_constraint) {
        // Runtime path change — forward to live Jolt constraint
        if (path) {
            auto joltPath = path->createJoltPath();
...
void PathConstraint::setPositionMotorSettings(MotorSettings *settings)
{
    ...
    if (m_positionMotorSettings) {
        m_motorSettingsConnection = QObject::connect(
            m_positionMotorSettings, &MotorSettings::changed, this, [this] {
...
void PathConstraint::setTargetVelocity(float velocity)
{
    ...
    if (m_constraint)
        static_cast<JPH::PathConstraint *>(m_constraint)->SetTargetVelocity(
            m_targetVelocity / 100.0f);
...
void PathConstraint::updateJoltObject()
{
    ...
    if (!m_path) {
        qWarning() << "PathConstraint: 'path' property is required";
        return;
    }
    ...
    m_constraintSettings.mPathPosition = PhysicsUtils::toJoltType(m_pathPosition / 100.0f);
    ...
    if (m_positionMotorSettings)
        m_constraintSettings.mPositionMotorSettings = m_positionMotorSettings->getJoltMotorSettings();
    ...
    if (m_positionMotorState == MotorState::Velocity)
        pc->SetTargetVelocity(m_targetVelocity / 100.0f);
