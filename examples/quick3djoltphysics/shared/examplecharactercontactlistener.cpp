...
int ExampleCharacterContactListener::characterBlockID() const
{
    return m_characterBlockID;
}

void ExampleCharacterContactListener::setCharacterBlockID(int blockID)
{
    if (m_characterBlockID == blockID)
        return;

    m_characterBlockID = blockID;
    emit characterBlockIDChanged(m_characterBlockID);
}
...
void ExampleCharacterContactListener::onCharacterContactAdded(const Contact &contact, CharacterContactSettings &settings)
{
    settings.canPushCharacter = false;
}

void ExampleCharacterContactListener::onCharacterContactSolve(const Contact &contact, const QVector3D &contactVelocity, const QVector3D &characterVelocity, QVector3D &newCharacterVelocity)
{
}
